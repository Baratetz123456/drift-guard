...
> [!IMPORTANT]
> ### DriftGuard Precedence & Brand Guardrail
> When operating within the DriftGuard repository, **DriftGuard Brand & Visual Standards (`brand-driftguard` and `AGENTS.md`) take absolute precedence** over generic aesthetic defaults:
> 1. **Brand Accent**: Pure Voltage (`#c8ff00`) in dark mode with dark text (`text-zinc-950 font-bold`); Lime-700 (`#4d7c0f`) in light mode.
> 2. **Zero-Emerald Law**: Standard emerald and green tokens are strictly eliminated. Verified/success states and diff additions use Voltage (`#c8ff00`).
> 3. **Monochrome Zinc Surfaces**: Dark obsidian `slate-950`/`zinc-950` backgrounds with flat OpenRouter-inspired zinc containers (`bg-zinc-950`, `bg-zinc-900/60`, `border-zinc-800`). Zero card/button gradients.
> 4. **Universal Fluid Detail Layout**: All detail and profile views MUST render fluid full-width (`w-full space-y-6 font-sans`), never boxed in narrow centered wrappers.
> 5. **Senior Engineer Copy & Button Naming**: Strict sentence case, zero exclamation marks, 1–2 word action buttons (`Run collection`, `Compare`), and 3-part error diagnostics (*Observation*, *Operational impact*, *Actionable next step*).
